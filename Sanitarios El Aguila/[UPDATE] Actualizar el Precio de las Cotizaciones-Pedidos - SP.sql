...
WHERE GVA21.ESTADO IN ('1','2') --Filtro por los pedidos "Ingresados" y "Aprobados", solo esos afectará.
    ...
    AND (
        ...
        (GVA08.FECHA_ALTA = GVA21.FECHA_PEDI 
		AND DATEDIFF(MINUTE, CONVERT(DATETIME, GVA08.FECHA_ALTA + ' ' + STUFF(STUFF(GVA08.HORA, 5, 0, ':'), 3, 0, ':'), 120), GETDATE() ) >= 30 ));-- La siguiente condición filtra registros donde la diferencia en minutos entre la fecha/hora de alta y la fecha/hora actual es mayor o igual a 30. Esto se hace para incluir o excluir registros según si ha pasado más de 30 minutos desde cierto evento registrado en la tabla de Cotizaciones.
